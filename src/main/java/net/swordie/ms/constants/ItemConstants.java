...
//TODO: adding consumable drops for monsters per lvl
    ...
    public static List<ItemOption> getOptionsByEquip(Equip equip, boolean bonus, int line, int cubeId, int additionalPrimes) {
        ...
        ItemGrade grade = getLineTier(line, ItemGrade.getGradeByVal(bonus ? equip.getBonusGrade() : equip.getBaseGrade()), additionalPrimes);
        ...
        if (isShield(id) || isSecondary(id)) {
            ...
            if (isRing(id) || isPendant(id) || isFaceAccessory(id) || isEyeAccessory(id) || isEarrings(id)) {
                ...
                if(isGlove(id)){
                    if(grade == ItemGrade.HiddenUnique || grade == ItemGrade.Unique || grade == ItemGrade.HiddenLegendary || grade == ItemGrade.Legendary || grade == ItemGrade.UniqueBonusHidden || grade == ItemGrade.LegendaryBonusHidden){
